...
import { useRealtime } from "@/lib/realtime/conversation/use-realtime";
import type { RealtimeOptions } from "@/lib/realtime/conversation/types";
...
beforeEach(() => {
  ...
  vi.stubGlobal("fetch", vi.fn(async (_url: unknown, init?: RequestInit) => init?.method === "POST"
    ...
    : { ok: true, json: async () => ({ configured: true }) }));
});
...
describe("Realtime response ownership", () => {
  ...
  it("uses the newest context and handler when options change", async () => {
    ...
    const session = channel.events().find(event => event.type === "session.update")?.session as { instructions: string };
    expect(session.instructions).toContain("Updated guide");
    expect(session.instructions).toContain("selected: poi-2");
  });
});
