...
import { GET, POST } from "@/app/api/prototype/conversation/realtime/route";

const key = "test-secret-must-remain-server-side";
const validBody = { sdp: "v=0\r\n", instructions: "Test", tools: [], mode: "text" };

function request(url = "http://localhost:3001/api/prototype/conversation/realtime", origin?: string) {
...
describe("local Realtime session boundary", () => {
  it("keeps the credential in the upstream Authorization header and returns only SDP", async () => {
    ...
    const health = await GET(new NextRequest("http://localhost:3001/api/prototype/conversation/realtime"));
    expect(await health.text()).not.toContain(key);
  });

  it("rejects nonlocal and cross-origin requests before contacting OpenAI", async () => {
    expect((await POST(request("https://example.com/api/prototype/conversation/realtime"))).status).toBe(404);
  ...
  it("disables the paid session factory in production even on localhost", async () => {
    ...
    expect((await GET(new NextRequest("http://localhost:3001/api/prototype/conversation/realtime"))).status).toBe(404);
    expect(fetch).not.toHaveBeenCalled();
  });

  it("accepts the browser's loopback Host when Next normalizes its URL, but not a different origin", async () => {
    const normalized = (origin: string) => new NextRequest("http://localhost:3101/api/prototype/conversation/realtime", {
      method: "POST", body: JSON.stringify(validBody),
