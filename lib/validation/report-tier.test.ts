...
describe("validateReportTier — nivå 1", () => {
  it("nivå 1-config uten editorial består", () => {
    const findings = validateReportTier({
      slug: "basic",
      reportConfig: {
        reportTier: 1,
        themes: [theme("transport", { editorial: undefined })],
      },
    });
...
describe("validateReportTier — nivå 2", () => {
  ...
  it("highlight alene (tom body) teller som editorial", () => {
    const findings = validateReportTier({
      slug: "highlight-only",
      reportConfig: {
        reportTier: 2,
        themes: [theme("a", { editorial: { body: "", highlightPoiIds: ["poi-1"] } })],
      },
      poiIds: ["poi-1"],
    });
    expect(errors(findings)).toEqual([]);
  ...
  it("ortogonale akser (VO/3D/brokers/brand) sjekkes IKKE på nivå 2", () => {
    // Nivå 2 uten audio, has_3d_addon, camera-tours, brokers eller brand —
    // alt skal være grønt fordi editorial er den eneste nivå-2-gaten.
    const findings = validateReportTier({
      slug: "editorial-only",
      reportConfig: {
        reportTier: 2,
...
describe("validateReportTier — ugyldig deklarasjon", () => {
  it.each([[4], ["2"], [0], [3]])("reportTier %j avvises", (bad) => {
    const findings = validateReportTier({
      slug: "bad",
...
describe("summarizeTierFindings", () => {
  it("oppsummerer i «deklarert nivå 2, mangler: …»-stil", () => {
    const findings: ReportTierFinding[] = [
      { level: "error", check: "editorial", detail: "x" },
      { level: "error", check: "editorial", detail: "y" },
      { level: "warning", check: "highlight-poi", detail: "w" },
    ];
    expect(summarizeTierFindings(2, findings)).toBe(
      "deklarert nivå 2, mangler: editorial",
    );
