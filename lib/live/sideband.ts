...
export interface SidebandSupervisor {
  setCleanup(token: string, cleanup: (reason: string) => void): void;
  end(token: string, reason?: LiveEndReason): Promise<boolean>;
}

export interface LiveSidebandOptions {
  hosted?: boolean;
  bridge?: MapBridge;
  supervisor?: SidebandSupervisor;
...
export async function connectLiveSideband(
  ...
  const supervisor = options.supervisor ?? getLiveSupervisor();
  ...
  const log = (line: string) => {
    if (options.hosted || process.env.NODE_ENV === 'test') return;
