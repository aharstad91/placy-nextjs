import { discoveryCategoryNames, quotedList } from "@/lib/demo/local-board/voice";
...
export function buildLocalVoiceInstructions(dataset: LocalDataset): string {
  ...
  // Stedsordet står i datasettet. «Bydelen» var Nyhavnas ord, og et
  // boligprosjekt har ingen bydel som kommer.
  const afterGreeting =
    voice?.afterGreeting ?? "La brukeren velge det som kommer eller stedene som finnes i dag. Ved planene, deleger valget.";
  const planScope = voice?.planScope ?? "Skill mellom planene og tilbud som finnes i dag.";
  // Kategoriene som tåler radiusutvidelse står i datasettet. Har det ingen,
  // sies setningen ikke: en regel uten innhold inviterer til å finne på data.
  const discovery = discoveryCategoryNames(dataset);
  const discoverySentence = discovery
    ? `For ${discovery} kan backenden utvide radiusen med to kilometer om gangen og vise nye steder. `
    : "";
...
Flere steder: Når brukeren ønsker flere alternativer i samme kategori, deleger forespørselen én gang. ${discoverySentence}La avstandsstyringen skje i bakgrunnen. Ikke les opp radius eller antall steder med mindre brukeren spør. Si «Dette er noen av alternativene i nærheten. Vil du se flere lignende steder i nærheten?» Nevn ALLE nye steder som backenden returnerer, i oppgitt rekkefølge, med én kort beskrivelse og en tydelig pause per sted. Bruk kategorien i spørsmålet når det passer, for eksempel «flere lignende treningssteder». Vent på et tydelig ja før neste innhenting; tilbakemeldinger om formulering er ikke en bestilling av flere steder. Si «Jeg viser deg noen flere», ikke at du søker på nettet. Tilby dette når backenden sier det finnes flere. Er utvalget tomt, ikke tilby å lete igjen.
...
${planScope}${roleSentences} Bruk bare fakta backenden har gitt. Si stedets navn sammen med reisetid og reisemåte. Skill gangtid til holdeplass fra busstid. Behold nødvendige forbehold, uten å gjenta samme forbehold flere ganger. Fortell om stedet, uten å innlede hvert svar med hva du har gjort i kartet. Ikke si at noe er vist før backenden har bekreftet det.
