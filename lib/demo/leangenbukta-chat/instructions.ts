...
export function replyNotice(input: { userText: string; reply: string; answerType: string; provisional: boolean }): ReplyNotice | null {
  ...
  const kind: ReplyNoticeKind | null = SALES_WORDS.test(text)
    ? "sales"
    : TIMING_WORDS.test(text)
      ? "timing"
      : input.provisional && input.answerType === "fact"
        ? "provisional"
        : null;
  return kind ? { kind, text: NOTICE_TEXT[kind] } : null;
}
