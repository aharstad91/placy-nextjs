...
/**
 * PRD 14 Unit 3 (r14.3) kildekontrakt-guard for tour-byggeren.
 *
 * `audio-tour-build.ts` (DB) er en port-with-rewrite build-time CLI.
 * tsc + lint fanger typene og import-stiene, men IKKE AC-invariantene under —
 * to-fase-mønsteret, den optimistiske låsen, audioVersion=5-aksen på tvers av
 * byggerne og revalidate-taggen. Vi leser kilden som tekst i stedet for å
 * importere modulene, fordi top-level CLI-koden kaller `process.exit()` på
 * manglende argv (samme mønster som curate-narrative.contract.test.ts).
 * (Lokal-JSON-byggeren døde med data/projects/ i legacy-oppryddingen
 * 2026-07-06 — AC1/AC4-kontraktene dens gikk med den.)
 */
...
describe("r14.3 AC1 — to-fase: Phase 1 TTS parallelt in-memory + MIN_BYTES, abort før disk-write", () => {
  it("PARALLEL_LIMIT=2 (ElevenLabs free-plan-tak)", () => {
...
describe("r14.3 AC3 — audioVersion=5 konsistent på tvers av byggerne", () => {
