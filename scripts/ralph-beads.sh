...
4. GRØNNE gates → `bd close <id>` med dokumenterende kommentar + `bd export -o .beads/issues.jsonl` + `git add` + `git commit` (Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>). ALDRI `git push`. Oppdater memory-frontier kort.
