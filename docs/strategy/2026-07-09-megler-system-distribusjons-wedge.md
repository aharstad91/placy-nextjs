...
4. **Inkumbenten er slåbar på dybde, ikke på pris/tilstedeværelse.** Nabolagsprofilen er avstand-til-POI, ikke-redaksjonell, ingen lokalkunnskap, ingen 3D/visuelt lag. Placys Moat 1 + Innsikt + immersjon er det strukturelle gapet.
   > **⚠️ KORRIGERT 2026-08-04 etter faktisk benchmark på egen adresse** (`2026-08-04-objektside-veggen-hem-og-nabolagsprofil-benchmark.md` §6). Denne formuleringen er for optimistisk. Profilen har **gangtid til navngitte skoler, barnehager, holdeplasser og idrettsanlegg**, **solforhold** (terrenghorisont via Geodata), demografi, reisetid-verktøy og **et eget brukergenerert lokalkunnskap-lag** («vurdert av 82 lokalkjente» på Ranheim). Reisetidene deres kommer fra **Entur + Google** — samme kilder som Placys. Det de faktisk mangler er (a) **stedene** (ikke én kafé, bakeri, strand eller butikk i produktet), (b) **målingen tilbake til megleren**, og (c) presisjon (tre demonstrerbare feil funnet på Østmovegen 6 D, inkl. kjøretid ført under «gangavstand til skole» slik at feil skole leses som nærmest). Score-primitivet er deres bærebjelke og Placys differensieringsakse — tre av fem scorer lå 1–6 poeng fra bysnittet, og «meget godt kollektivtilbud» sto på en score *under* snittet. **Ikke pitch «vi er dypere» generelt — pitch stedene og målingen.**
...
| **EiendomsMegler 1** | Allianse: 12 selvstendige regionale SpareBank1-/Eika-selskap | ~~Ukjent~~ → **Webtop Solutions / Visma Real Estate** for **EM1 Midt-Norge** | **BEKREFTET 2026-08-04** — objektsidene laster case-data fra `em1.webtopsolutions.com/flow/api/Public/Cases/<guid>`, bud + visningspåmelding på `…/tide/`. Webtop ble del av Visma Real Estate i 2021. **Gjelder verifisert bare Midt-Norge; de 11 andre regionselskapene kan ha andre valg.** |
...
Én partner-integrasjon mot Next HUB → embeddet på tvers av 8+ bekreftede kjeder samtidig.

> **⚠️ VIKTIG PRESISERING 2026-08-04:** «embeddet på tvers av kjeder» holder ikke som mekanisme uten forbehold. **Kjedens nettsted er ikke fagsystemets.** Verifisert: `eiendomsmegler1.no` er en **SpareBank 1 Felles Front End**-plattform (delt av alle 12 regionselskapene) med **0 iframes og 0 `dangerouslySetInnerHTML`** på 153 annonsesider — mens fagsystemet bak er Webtop/Visma. En fagsystem-integrasjon plasserer Placy i **meglerens arbeidsflyt**, i **det som syndikeres** (FINN-annonse, bilder, prospekt) og potensielt i **salgsoppgaven** — men *hver kjedes nettsted må fortsatt rendre flaten selv*. Systemleverandøren er altså en **distribusjonspartner, ikke en embed-nøkkel**. Sekvens-konsekvens: enhver systemleverandør spør «hvor mange av våre kunder ber om dette?» → **partner-sporet krever minst én levende referansekunde først.** Detaljer: `2026-08-04-objektside-veggen-hem-og-nabolagsprofil-benchmark.md` §1 + §7b. Dette er den ekte «Local Logic rir MoxiWorks»-analogen, og et **nytt strategisk alternativ ved siden av «vinn én kjede»** (06-27). Teknisk risiko lav (etablert partner-program, presedens i Kvass/Boligvelger/Ambita). Handelen man må vinne: en plass i systemet der FINN Nabolagsprofil allerede ligger → **fortrengning/premium-oppsalg, ikke greenfield**.
...
## Åpne spørsmål
1. ~~Er Eiendomsprofil AS uavhengig (oppkjøps-/partner-kandidat?) eller FINN-intern?~~ → **PEKER MOT FINN-OPERERT (2026-08-04).** `nabolag.no` redirigerer til `finn.no/nabolag/sporsmal`, og profilens egen kildeside navngir **Finn.no som operatør** («For å oppnå høyest mulig kvalitet har Finn.no i tillegg flere distribusjonspartnere som også kan gi tilbakemeldinger»). Brukergenererte vurderinger samles inn *via FINN*. **Konsekvens: en megler som embedder nabolagsprofilen på sin egen objektside embedder FINNs produkt** — «bytte leverandør» betyr å bryte med FINN, ikke med en liten Bergen-leverandør. Oppkjøps-hypotesen er dermed svak. Detaljer: `2026-08-04-objektside-veggen-hem-og-nabolagsprofil-benchmark.md` §6.
