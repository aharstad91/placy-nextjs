...
**Begrunnelse:** Andreas' vurdering etter tre øvingsrunder: 4 490 er en «levbar» recurring som lander flere prosjekter, og mersalget skal komme fra moduler og andre leveranser — ikke fra en høy grunnpris. Kontra-argumentet (Placy under Propti = priset på pris, ikke verdi) ble veid og lagt bort: Propti er feil målestokk uansett — HEM har alt Marketer-boligvelger og egen nettside, og den reelle konkurrenten for beliggenhets-jobben er en JPEG fra egen designer (kost ≈ 0). I pre-kunde-fasen er én referansekunde verdt mer enn 1 500 kr/mnd i margin. Kjent kostnad ved valget: beslutningsregelen «ja uten å blunke = for lavt» gir mindre prissignal på dette nivået — akseptert; læringen flyttes til modulsalget.

**Betingelse — modulene må ha navn og grov pris før Thomas-møtet**, ellers er det ingenting å ekspandere til. Kandidatene ligger alt i produktet/tier-modellen: omvisning med stemme (nivå 2), 3D-modell av bygget (krever utbyggers IFC/GLB, ikke bygd), flyover-film/Reels til SOME, flere flater (FINN «Nyttige lenker» med `?src=`, QR på papir, karusellbilde), porteføljeinnsikt på tvers av prosjekter (den eneste analysen som prises separat). Prissetting av modulene = neste materiell-oppgave sammen med priskortet.

**Målregnestykket (korrigert):** ett nytt prosjekt = 24k + 12 × 4 490 ≈ 78k første år, ett løpende ≈ 54k/år → **~15 løpende prosjekter** bærer 800k-målet alene (var ~11 ved 6 000). Mixen står: 5 løpende (~270k) + én kjede-grunnpakke på brukt (HEM alene 0,85–1,4M) passerer målet. Modulsalget skal tette gapet mellom 54k og 72k per prosjekt.

**Detaljer:** Strukturen og enhetsregelen i `docs/strategy/2026-09-01-prisvalidering-bevispakke.md` §7 (tallene der er nå historikk; strukturen gjelder).

**Status:** Aktiv. Supersederer månedssatsen i entry 2026-09-01 (kveld); oppsett, kvartalsfakturering, fornyelsesmotor og volumtrapp-prinsippet står.

---

## 2026-09-02 — Analysen er inkludert i månedsprisen, ikke egen SKU — og Møte-1-manuset fikk sine tre regler

**Beslutning:** Innsikts-/analysedelen (hvilke kategorier, steder og spørsmål som får oppmerksomhet på boardet) faktureres IKKE separat. Den ligger inne i prosjekt-SKU-en (oppsett 24 000 + løpende månedspris per prosjekt-board — 4 490 fra 2026-09-02 forts., opprinnelig 6 000 jf. 2026-09-01 kveld). Det eneste som kan prises separat, senere, er **aggregering på tvers av prosjekter** — portefølje-/områdeinnsikt (Brøset-skala, målgruppe-validering for hele porteføljen) — og det hører hjemme i porteføljeavtalen/250k-områdeformen, ikke i enkeltprosjektet. Møte-formulering: «Analysen følger med — det er slik dere ser at dette virker.»

**Begrunnelse:** Datagjennomgangen i måned 3 og 6 er fornyelsesmotoren (2026-09-01 kveld) — Placy er eneste leverandør på utbyggerens faktura som kan bevise egen verdi. Skilt ut som egen linje kan den velges bort, og da forsvinner både fornyelsesargumentet og datastrømmen til Moat 2 (Innsikt). I et marked der utbyggerne kutter digitale budsjetter (2026-09-01: «selg omfordeling, ikke friske penger») er en ekstra linje det første som strykes. Marginalkostnaden er null: hendelsene måles alt, analysen er en rapport, ikke et prosjekt. Spørsmålet kom opp fordi Andreas i pitch-øvelsen ber Thomas om hjelp til å forme analysekonseptet — og man kan ikke fakturere separat for noe man ber kunden ko-designe; ko-designet ER pilotens verdi.

**Addendum — Møte-1-manuset (to øvingsrunder på Wesselsløkka-pitchen, 16 min → 10 min, mål 6–7 min):** Tre regler festet. (1) **Ingen hedger** — «produkt vi bygger as we go», «vi er fortsatt i en fase», «vi har kommet langt, det har vi» strykes; én setning: «Dette er klart til bruk og kan ligge på wesselslokka.no denne uka» (embedden er minutter, kurateringen med deres input tar et par uker — si det slik, ikke «noen uker» om alt). (2) **Slutten har pivot + ask** — Wesselsløkka er 15 mnd inn i salget og går godt uten Placy, så: «På neste prosjekt er dette live fra salgsstart» + «kan du ta meg med til Einar i 20 minutter?». (3) **Én demo-tråd** — familie → skole → skolevei (11 min gå / 4 min kjøre) → barnehage → idrettsplass, ett transport-slag på 30 sek, stopp; ikke improviser over bysykkel/bussruter. Språkregel: «megleren som er tilgjengelig 24/7», aldri «erstatter jobben deres» / «digitaliserer megleren» — ingen kjøper sin egen erstatning (og «vi sparer dere arbeid» er alt dødt mot markedsavdelingen, 2026-08-05). Det som sitter og skal beholdes: åpningen på at HEM alt lager statiske JPEG-kart på Wesselsløkka/Solsletta, «svare spørsmål før de blir spørsmål», «det er dere som er avsender», og å gi Thomas en rolle (analysekonsept + prioritering av spørsmål per kategori) så han tar det inn til Einar. Forutsetning før møtet: overblikkskartet må ikke se kaotisk ut (739 pins flatt rangert på Google-rating — brainstorm om lagdeling påbegynt 2026-09-02, ikke landet), og datafeilene som vises live («Athletic Field», Geo Seabed som frisør) må fikses. Ha prissvaret klart hvis han spør: oppsett 24 000 + 4 490/mnd (se entry 2026-09-02 forts.).

**Detaljer:** Prisstruktur og fornyelsesmotor i `docs/strategy/2026-09-01-prisvalidering-bevispakke.md` §7; Thomas-prosessen i LOG-entry 2026-09-01 (kveld).

**Status:** Aktiv.

---

## 2026-09-01 (kveld) — Prisstrukturen landet: oppsett + løpende månedspris, 800k-målet tallfestet, og Thomas-prosessen designet

**Beslutning:** Prosjekt-SKU-en presenteres heretter som **«oppsett 24 000 + 6 000/mnd, kvartalsvis fakturert, løpende til prosjektet er utsolgt»** — matematisk identisk med 60k/6 mnd første halvår, men strukturen speiler markedet (Propti: oppstart + løpende), gjør fornyelsen etter måned 6 selvsagt i stedet for et forhandlingspunkt, og fjerner tallet «10 000/mnd» som ikke overlever enhets-vask mot ekte månedslinjer (FINN 7 800/mnd, Propti 4 490/mnd — flate 10k lå over alle; 4 990/mnd ble vurdert og frarådet fordi det legger Placy *under* Propti og konkurrerer på pris i segmentet som skal prises på verdi). Fornyelsesmotoren er innebygd: datagjennomgang måned 3 og 6 (Moat 2-tall — Placy er eneste leverandør på fakturaen som kan bevise egen verdi), og måned 6-gjennomgangen er samtidig åpningen av bruktbolig-/grunnpakkesporet. Volumtrapp besluttet i prinsipp: 2–4 prosjekter ~−8 %, 5+ ~−17 %, og rabatt kjøper alltid noe (forpliktelse, lengde eller referanserett) — aldri gis fordi noen spør. «Placy på alle prosjektene våre» er ikke fem enkeltsalg: det graduerer til porteføljeavtale, eskaleres til systemeieren, og er 250k-områdeformens naturlige hjem.

**Målregnestykket (nytt):** Skal Placy erstatte arbeidsgiver kreves ≥800k fakturert/år. Med 24k + 6 000/mnd: ett nytt prosjekt = 96k første 12 mnd, ett løpende = 72k/år → **~11 løpende prosjekter bærer målet alene** (realistisk i Trondheim over et par år — titalls aktive nybyggprosjekter til enhver tid). Men mixen er raskere: **5 løpende prosjekter (~360k) + én kjede-grunnpakke på brukt (HEM alene = 0,85–1,4M ved 500–800/annonse) passerer målet med god margin.** Tesla-logikken står: prosjektene er broen, ikke hele veien — ett kjede-ja på brukt er verdt mer enn tretten prosjektsalg.

**Thomas-prosessen (HEM/Wesselsløkka) designet:** Møte 1 med Thomas Løbakk er IKKE et salgsmøte — kort, live board på hans eget prosjekt, ingen pris med mindre han spør; målene er engasjement og at han tar det inn til Einar Ringen Jr., derfor bygges materiellet for å *videresendes internt* (mobilperfekt lenke + én-sider). Møte 2 med Einar i rommet er salget: priskort, Clarity-spørsmålet («hvor mange som åpner en annonse, åpner Nabolagsprofil-fanen?»), embed-asken (Squarespace-blokk, minutter), og rammeavtale-spørsmålet (hvem betaler marked for dette prosjektet — Heimdal Bolig eller HEM; dokumentert at fakturaen normalt treffer utbygger). Deretter: data mnd 3 → porteføljesamtale (flere prosjekter/Brøset) → bruktbolig/grunnpakke → Vitec-introen. Forventningsjustering: Wesselsløkka er 15 mnd inn i salget og gikk godt — lavt hastverk der; pitchen peker fremover: «på neste prosjekt er dette live fra salgsstart» (fase-funnet: pengene brukes tyngst FØR byggestart).

**Neste steg (rekkefølge):** (1) de to blokkerne fra 2026-08-05 — iPhone-verifisering av eget Wesselsløkka-board + radius-drift-opprydding i boardets tekster; (2) materiell — priskort (ny struktur + volumtrapp + innvendingssvar) og delbar kort-pitch; (3) kontakt Thomas.

**Metoderegel festet (enhetsregelen):** sammenlign totaler mot totaler innenfor samme periode, eller månedspris mot månedspris — aldri på tvers. Regelen ble til ved at en engangspris (BlikkFang-runde) ble brukt til å underbygge et per-måned-argument og falt ved utfordring. Gjelder alt prismateriell og alle tallargumenter.

**Detaljer:** `docs/strategy/2026-09-01-prisvalidering-bevispakke.md` §7 (enhetsregelen + strukturen).

**Status:** Aktiv — struktur og arbeidstall besluttet; volumtrappens eksakte satser finjusteres når priskortet bygges.
Supersedert 2026-09-02 (delvis) — månedssatsen 6 000 erstattet av 4 490, se entry 2026-09-02 (forts.). Struktur, oppsett 24 000, kvartalsfakturering og fornyelsesmotor står.

---

## 2026-09-01 — Prisvalidering med bevispakke: 60k/6 mnd HOLDER, 250k omdefinert til områdeform — og prisvåpenet deles i to

**Beslutning:** Prosjekt-SKU-ens tre tall er validert mot dokumenterte norske markedspriser (tre parallelle research-spor: visuelt innhold, digitale salgsverktøy, budsjettnorm — 43 verifiserte norske prispunkter + FINN-prislisten 2026 + reviderte regnskap). **Tilbud 60k/6 mnd står** for prosjekter på 100+ enheter: Propti Partner (nærmeste parallell, komplett digital salgsplattform 80–199 enheter) koster 66 840 for seks måneder; prosjektets egen FINN-annonse koster ~94 500 over en 18-måneders salgsperiode; 60k ≈ 1,5 % av prosjektets sannsynlige markedsbudsjett. **Gulv 40k står.** **250k/24 mnd skal ALDRI presenteres som pris på ett prosjekt** — ingenting i det norske markedet på ett-prosjekt-nivå er i nærheten, og tallet taper troverdighet mot Propti-/Kvass-kjennere; det er område-/flertrinnsformen (Brøset-skala). Ankeret i rommet forblir Marketer-linja (50–200k). I tillegg landet en ny prisstrategi-beslutning: **prisvåpenet deles i to** — premium prises på verdi (nullkost-strukturen brukes aldri til å underprise prosjekt-SKU-en), prisaggresjon hører hjemme i massemarkedet/grunnpakka (500–800/bolig) der kostnadsstrukturen er umulig å matche for aktører med lønnskostnader.

**Begrunnelse (hovedfunn):** (1) Markedet har to prisbånd — tillegg 5–27k og plattform 33–67k; 60k tvinger Placy inn i plattformbåndet, så pitchen må være «konkurrerer om samme kroner som noe dere alt kjøper», aldri «et fint tillegg». (2) Budsjettnorm-korreksjon: «1–3 % av salgsverdi til markedsføring» er bedriftsnorm, ikke prosjektnorm — megler+marked samlet ≈ 2 % (Selvaag Bolig årsrapport 2025: 1,99 %; KDD-utredning 34-2022), ren markedsføring 0,4–1,0 %; markedsbudsjett for 122 enheter ≈ 3–4 MNOK (to metoder konvergerer). (3) Nabolagsvinkelen er DØD som salgsargument — FINNs Nabolagsprofil er bundlet gratis i pakker utbyggeren alt kjøper; Placy selger salgsverktøy-jobben (interaktivt, oppdaterer seg, måler). (4) Kategorien blør: Vergence/Marketer aldri overskudd på seks år (−299 MNOK akkumulert, −15 % omsetning 2024, rebrandet til HomeKey des. 2025 med nybygg utskilt i eget selskap); Kvass (faktisk nybygg-markedsleder, 30 % av nyboliger, PrivatMegleren-eksklusiv) −8 MNOK i 2024 → selg omfordeling av eksisterende linje, ikke friske penger — og inkumbentenes økonomi er ødelagt for DEM, ikke for Placys agent-baserte byggemodell (~null marginalkost). (5) Under ~50 enheter gjelder eget prisbånd 20–30k (alle sammenlignbare ligger der) — Sundsøya-anslaget 25–40k er i øvre kant. (6) FINN fakturerer nybygg per måned og prosjektannonser ligger i snitt over ett år → abonnement-innvendingen er empirisk død, og 6 mnd er kort mot reelle kampanjer (12–30 mnd) — fornyelse er innebygd. (7) Fakturaen treffer normalt utbygger, ikke megler (NEF-rammeavtale, Selvaag-stillingsannonse, OBOS-regnskapspraksis) — hvem som betaler marked for et gitt prosjekt står i rammeavtalen og må spørres om, ikke antas. (8) Pipeline-regel: pengene brukes tyngst FØR byggestart (forhåndssalgskrav 40–80 %) → beste inngang er teaser-/salgsstartfasen; Wesselsløkka (15 mnd inn i salget) er døra inn til HEM, ikke malen for kundeprofilen. (9) Grunnpakke-prisen er konservativ: Marketer henter estimert 2–4k per bruktbolig i dag, og meglernes markedspakker (17 900–27 900/bolig, betalt av selger) er den naturlige bæreren av en 500–800-linje.

**Beslutningsregler for møtene (validering fortsetter i markedet):** ja uten å blunke på 60k = for lavt, juster opp neste gang · forhandling som lander på gulvet (40k) = riktig sone · nei på pris tross Marketer-ankeret = reelt datapunkt for revurdering. Estimater (Kvass-/Marketer-snittpriser, antall renders, fysisk modellpris, Fastout-pris) skal aldri siteres som fakta i kundemateriell — merket i vedleggene.

**Addendum samme dag — Plyo:** Plyo ≠ Marketer (navnekollisjon: begge har produkt kalt «Property Explorer»). Plyo AS = fusjonen Plyo + norske 3D Estate (sept. 2023), 72 ansatte, hevder >70 % av norsk Enterprise-nybygg — manglet i vedlegg-digitalt (som feilaktig avskrev 3D Estate som polsk) og er trolig den reelle Enterprise-inkumbenten ved siden av Marketer. Regnskap 2024: 53,7 MNOK omsetning, **−10,5 MNOK driftsresultat** — tredje bekreftelse på at kategorien blør. Boligvelgeren på wesselslokka.no DOM-verifisert live: laster fra `newbuilds-assets.com` med `marketer-*`-iframe-id-er → **Marketer-attribusjonen fra 2026-08-04 står**. Metoderegel: sjekk script-domenet før leverandør-attribusjon.

**Detaljer:** `docs/strategy/2026-09-01-prisvalidering-bevispakke.md` (syntese, §1–§7 inkl. Plyo-addendum §6) + tre vedlegg: `2026-09-01-prisvalidering-vedlegg-visuelt.md`, `-digitalt.md` (inkl. Marketer/HomeKey-selskapsstruktur og regnskap — OBS: «Marketer AS» på proff.no heter nå Veyor AS og er feil selskap; de reelle tallene ligger på Vergence AS), `-budsjett.md`.

**Status:** Aktiv — validert på skrivebordet (listepriser, ikke transaksjonspriser); neste validering er de faktiske møtene via beslutningsreglene.

---

## 2026-08-13 — Moat 1 avgrenset: leverandør-tekst er stillas, ikke varelager. Lei det som forvitrer, eier det som akkumulerer — og det dypeste laget kan ingen skrape

**Beslutning/innsikt:** Startet som teknisk arbeid (innhold i Utforsk-modalen) og endte i en avgrensning av hva Moat 1 faktisk er. Utløst av Andreas' spørsmål: *«hvis vår moat 1 er at vi EIER kunnskapen, skyter vi oss i foten av å bruke leverandør-tekst da? Eller er vår moat 1 at vi har en EVNE til å formidle kunnskapen?»*

1. **Svaret er ingen av dem — og det første er falsifisert av vårt eget arbeid.** Å eie tekst er ikke forsvarbart: 59 stedstekster på Sundsøya kostet **én scriptkjøring** innenfor Geminis gratiskvote. Marginalkostnaden for en konkurrent å generere sammenlignbar tekst er nær null; FINN Nabolagsprofil har tekst, Google har tekst, alle med API-nøkkel har tekst. Og formidlingsevnen er **produktet, ikke vollgrava** — den har reell markedsverdi (HEM *kjøpte* Marketers boligvelger framfor å bygge, 08-04), men et bedre produkt er ingen vollgrav med mindre noe beskytter det. **Det som ikke kan kopieres er dømmekraften:** å vite hvilke 4 av 78 punkter betyr noe for en kjøper i Straumen, og at Straumtorget-teksten handlet om feil park. Det manifesterer seg som tekst, men *er* ikke teksten.

2. **Arbeidsprinsippet: lei det som forvitrer, eier det som akkumulerer.** Andreas' hygiene-poeng var riktig, men snudd — at Google eier datahygienen på det leide laget er en **fordel** vi skal beholde. Åpningstider, telefon, rating og bilder skal vi aldri eie: eier vi dem, eier vi forfallet (vi har alt bygd for leieforholdet — månedlig oppdatering av åpningstider, 14-dagers utløp på lh3-URLer). *«Offentlig tannklinikk i samme bygg som rådhuset»* forvitrer ikke → eier. *«Straumtorget er ikke Muustrøparken»* er en korreksjon → der dannes eiendelen. **Leverandør-laget er verken:** Gemini-teksten forvitrer (2-års lagringsgrense i vilkårene, kildene råtner) *og* er ikke vår, så vi bærer vedlikeholdsplikten uten en leverandør som vedlikeholder. **Konklusjon: stillas, ikke varelager** — jobben er å dekke boards billig i dag og vise hvor hullene er. Dekningsrapporten er mer verdt enn teksten den produserte. *(Falsifisert samme dag: min egen design bevarte `generated` under kuratert tekst «for at provider-swappen til Googles generativeSummary skal være mulig». Feil resonnement — ruller Google ut i Norge, henter vi den live, vi migrerer ikke gammel Gemini-output.)*

3. **Regnestykket som gjør kuratert-bare mulig.** Første innvending var 4 264 nabolags-POI-er i basen. Feil på to måter: (a) **vi trenger ikke tekst på alle** — det som vises fram er høydepunktene, 4–6 per tema × 6 temaer **per strøk**, delt av alle boards i strøket = 25–35 tekster per strøk, samme amortisering som strøk-editorialen (06-27); (b) **volum er ikke en innvending med agent-kapasitet** — Andreas presiserte dette som generell arbeidsmåte: 4 000 tekster er en kjøring i batcher. Reelle blokkere er kvoter, penger, irreversible mutasjoner og ting som krever menneskelig verifisering — ikke arbeidsmengde. Gevinsten er dessuten større enn dekningstallet: **et helt delsystem forsvinner** (ingen Google-attribusjon, ingen ToS-klokke, ingen råtnende kildelenker, intet verbatim-krav, ingen SSRF-sjekk), og ingenting publiseres som ingen har lest — i denne sesjonen skrev jeg seks tekster fra Gemini-materiale med **én ukontrollert kilde** og måtte advare om det.

4. **Den strukturelle grensa er det viktigste funnet.** Sundsøya endte på **9 kuratert, 63 med leverandør-tekst, 6 igjen** av 78. De 6 er igjen fordi de **ikke kan skrives fra offentlige kilder**: Nilsparken, Vaggplassen og Straumtorget har ingen publisert informasjon noe sted — ikke hos Inderøy kommune, ikke i OSM, ikke hos Visit Innherred, ikke på dgo.no. De kan verken skrapes eller genereres. **Det gjør megler-forfatterskapet til design, ikke nødløsning** (08-04), og det er den sterkeste versjonen av moat-argumentet: det dypeste laget er strukturelt utilgjengelig for enhver konkurrent med et API-abonnement — **inkludert Google.** Google kan generere tekst om alt som er publisert; ingen kan generere tekst om det som ikke er. Forsterker rural-asymmetrien (08-11): ruralt ser området tomt ut hos FINN fordi det *er* tomt i offentlige data — der er gapet mellom «hva API-et vet» og «hva som faktisk er der» størst, og det gapet er det Moat 1 selger.

5. **Datahygiene som Moat-1-biprodukt — og det salgbare argumentet.** Kurateringen avdekket tre feil ingen automatikk fanget: **Straumtorget** (generert tekst handlet om Muustrøparken — porten stoppet den fordi den hadde 1 kilde, ikke fordi den var feil), **Sundsøya Ungdomsklubb** (var i ferd med å legges ned jan. 2024 pga. husleieøkning, senere reversert av huseier; om den driver i 2026 er uavklart — og den ligger på **selve prosjekttomta**, så en feil der ser kjøperen først → ikke skrevet), og **Studio F** (Vennavegen 10 i basen, Vennalivegen 2 i salongens egne kanaler). Dette er argumentet som gjør Moat 1 salgbart **uten å love dybde vi ikke har** (jf. korreksjonen 08-04 om at «slåbar på dybde» er for optimistisk): *vi retter feil i annonsens nabolagsdata.* Konkret, verifiserbart, og megleren ser det selv.

**Besluttet rekkefølge:** (1) behold leverandør-teksten på boardene nå — slår vi den av i dag går Sundsøya fra 72 til 9 synlige tekster; (2) kuratér høydepunktene per strøk, der agentene tar POI-ene som *har* offentlige kilder og `no-data`-listene går til megler; (3) **så** slå av rendringen av leverandør-laget, slik at Gemini-teksten blir kurator-input (som dossieret) som ligger i basen men aldri publiseres.

**Begrunnelse:** Moat 1 var navngitt (06-27) men ikke avgrenset — «vi eier lokalkunnskapen» tålte ikke at vi samme dag genererte 59 stedstekster gratis. Avgrensningen gjør moaten mindre, men ekte: dømmekraft og korreksjoner i stedet for tekstvolum, og en grense som er strukturell framfor bare kostbar å kopiere.

**Detaljer:** `docs/strategy/2026-08-13-moat-1-avgrensning-leverandortekst-vs-eid-kunnskap.md` — full analyse, lag-tabellen, research-sporet på de seks uskrivbare POI-ene.

**Status:** Aktiv — prinsippet gjelder, rekkefølgen besluttet. Åpent: (a) **når flippes bryteren** for å slutte å rendre leverandør-tekst (krever kuratert dekning på høydepunktene i minst ett strøk; ikke tidfestet); (b) **de 94 kategori-funksjonstekstene** — idéen som startet sesjonen («tannklinikk → hva gjør den? → fikser tenner») er ikke bygd, og med kuratert-bare som retning er det uavklart om de er nødvendige eller om høydepunkt-kurateringen dekker behovet; (c) **forfatterskap + auth for megler-laget** mangler fortsatt (08-04); (d) **driver Sundsøya Ungdomsklubb i 2026** — blokkerer tekst på et POI midt på prosjekttomta.

---

## 2026-08-12 — Fastout kartlagt: bildeselskap i «Nyttige lenker»-slotten, ~1 295 kr/bolig — nabo, ikke konkurrent

**Beslutning/innsikt:** Andreas fant en FINN-annonse (finnkode 446069340) der «Nyttige lenker» → **«Områdevideo»** peker på `apps.fastout.com/DynamicDroneView.aspx?panoguid=…`. Verifisert i browser. Andreas' vurdering, som står: **ikke en direkte konkurrent** — men de sitter i vår slot og på vår budsjettlinje, så de skal være kartlagt.

1. **Det er ikke en video.** Det er et **statisk 360°-dronepanorama** (Marzipano-viewer, ett stillbilde du kan dra rundt i / zoome / fullskjerme). Posisjon `63.424785, 10.408563` (Singsaker/Kristiansten, Trondheim). Seks **håndsatte** hotspots: objektet (grønn pin, «Til salgs! Velkommen på visning.»), Trondheim Sentrum 1 km, Solsiden 2 km, Tyholttårnet, Kristiansten festning, NTNU Gløshaugen. Ingen kategorier, ingen reisetid, ingen oppdatering, ingen måling.
2. **Pris: 1 295 kr eks. mva per stk, self-serve.** `/consumer/PaymentDroneView.aspx` — Stripe/Visa/MC, levert som lenke på e-post, ingen angrerett. Merk to forbehold: `culture=sv-SE` → tallet er trolig **SEK, ikke NOK** (samme størrelsesorden uansett), og dette er **ad-hoc-sporet** — kjedeprisen deres er nesten sikkert lavere. Tallet er derfor et **tak** i kategorien, ikke markedsprisen.
3. **`panobankguid` ≠ `panoguid` → de selger ut av en panorama-bank.** Forhåndsfløyne panoramaer gjenbrukes på flere boliger i samme område. Andreas bekrefter dekningen er bred («de har flydd over mitt eget hus»). Det er **samme amortisering som Placys per-strøk-kurering** (én kostnad, mange listings) — modellen er kommersielt validert, av noen andre, i vår slot.
4. **Svensk selskap med kjedeavtaler.** `fastout.com` → `fastout.se`, ingen `.no`-domene. Kundelista: Fastighetsbyrån, Skandinavisk Fastighetsförmedling, Bjurfors, HusmanHagberg, Mäklarringen. Det er «vinn ÉN kjede»-tesen utført i Sverige — og en advarsel om at en aktør med etablerte kjedeavtaler kan utvide nordover uten å bygge noe nytt.
5. **De er i en annen jobb: bilde, ikke data.** De svarer «hvordan ser det ut herfra», vi svarer «hva finnes her, hvor langt unna, hva betyr det for deg som skal bo her». Det de har er **bildedekning**, ikke kunnskap — pinnene er landemerker en person har plassert for hånd.
6. **Placys 3D-kart er allerede et supersett av produktet deres** (Andreas' poeng): Google-fotorealistisk 3D + flythrough lar deg fly **hvor som helst**, ikke se fra ett fast dronepunkt, og pinnene er datadrevne, ikke håndsatte. Vi har altså formatet deres som en birolle i et produkt som gjør mer.
7. **Priskonsekvens (intern, aldri pitch):** kontoravgift 15–25k/3 mnd mot et kontor med ~200 salg/år ≈ 50 boliger i perioden → **300–500 kr/bolig, altså under ett Fastout-panorama**. Brukes kun som svar hvis noen sier «vi har jo Fastout» — per-bolig-regning foran megler er fortsatt feil ramme (jf. 08-04).
8. **Selvbetjent kjøp av områdeinnhold er validert.** Megler legger inn kort og kjøper uten innkjøpsprosess. Det støtter `feat/megler-self-serve` (adresse → board → delingslenke), som ligger ubrukt i worktree.

**Forbehold som skal med:** **det rurale asymmetri-argumentet (§5f) treffer ikke Fastout.** FINNs Nærområdet-kart blir tomt på Inderøy — et dronefoto blir det ikke. Mot Fastout må argumentet være *data vs. bilde*, aldri *dekning*.

**Begrunnelse:** «Nyttige lenker»-slotten ble kartlagt 08-11 som teknisk ledig på bruktmarkedet. Fastout viser at den ikke er kommersielt tom: noen selger allerede «området rundt boligen» inn i den, til en observerbar pris. Det endrer ikke sporvalget, men det gir oss et prisanker i kategorien og en salgssituasjon å være forberedt på.

**Detaljer:** `2026-08-04-objektside-veggen-hem-og-nabolagsprofil-benchmark.md` §3c. Aktør-rad lagt i `aktor-map.md` (proptech-benchmark).

**Status:** Aktiv som kartlegging — **ingen handling, ingen kontakt.** Samarbeid ikke aktuelt nå (Andreas), men de sitter på norsk flyfoto-dekning + svenske kjedeavtaler, så partner-vinkelen holdes åpen i bakhodet.

---

## 2026-08-12 — Navnealternativ til «Placy» utforsket (gresk-rot-øvelsen): «Chora» er frontrunner, chora.no er ledig — PARKERT

**Beslutning/innsikt:** Andreas ville kjøre Anthropic-øvelsen (gresk rot som beskriver kjernen: *anthropos* → Anthropic) på Placy-navnet. Ingen beslutning om bytte — utforskning parkert her.

1. **«Placy» er allerede gresk uten at det var meningen:** «place» stammer fra *plateia (hodós)* — «den brede gaten»/torget. Øvelsen er altså ikke «finn et gresk navn», men «finn et som bærer mer mening».
2. **Kjerne-roten er sted, ikke kart/bolig.** Kandidatene som ble stående: **Chora** (χώρα — Platons «rommet der ting blir til»; *chorografi* = antikkens sjanger for detaljert beskrivelse av ett område, i motsetning til geografi — bokstavelig talt Placys sjanger), **Entopia** (*entopios* = «den lokale, av stedet»; moderne gresk *ntopios* = lokalkjent person; Doxiadis brukte Entopia om «det gode stedet som faktisk kan bygges» vs. utopia), **Nostos** (hjemkomsten — emosjonelt sterk for bolig, men peker bakover). Vurdert og lagt bort: Geitonia (= nabolag, men tung å uttale), Periego (Pausanias' *Periegesis*, verdens første reiseguide), Proxenos (vertskapsmannen som kjente byen for fremmede — konseptuelt finest, men kolliderer med «proxy»).
3. **Domenesjekk kjørt (Norid/whois 2026-08-12):** `chora.no` **LEDIG**, `entopia.no` **LEDIG**; `chora.com` opptatt siden 1998 men sovende/parkert (Cloudflare, svarer feilkode — trolig kjøpbar, ellers .io/get-mønster); `khora.no` og `nostos.no` tatt.
4. **Gjenstår før et evt. bytte er trygt:** varemerkesøk Patentstyret + EUIPO i relevante klasser (kjent nabo-kollisjon: Khora VR-studio i København), foretaksnavn i Brønnøysund, og uttale-test i pitch-setning («KO-ra»).

**Begrunnelse:** Navnebytte er ikke gratis — «Placy» er alt brukt i pitch-løpene mot HEM/EM1 og i self-serve-flatene (placy.no i kort-kode-ideen 08-11). Utforskningen er verdt å ha klar hvis rebranding blir aktuelt (f.eks. ved kjede-avtale eller investor-runde), men skal ikke drive arbeid nå.

**Status:** PARKERT — ingen beslutning, ingen registrering gjort. Hvis gjenopptatt: sikre chora.no + entopia.no billig først (~150 kr/år), varemerkesøk før noe annet.

---

## 2026-08-11 — FINN-annonsen har en klikkbar lenke-slot likevel: «Nyttige lenker» fra meglersystem-feeden

**Beslutning/innsikt:** Andreas fant en DNB-annonse med megler-skrevne, klikkbare lenker i FINN-appens «Nyttige lenker»-boks — «TRYKK FOR VIDEO» og «TRYKK FOR HJEMMESIDE» → `villahumlehaugen.no` (Villa Humlehaugen, Humlehaugvegen 37, Ranheim). Verifisert på den aktive salgsannonsen i **homes-vertikalen** (finnkode 468757276, prisantydning 42,9 M), synlig på både app og desktop.

1. **Mekanismen er en strukturert lenkekanal fra meglersystem-feeden — ikke et hack, og ikke fritekst.** Fritekst-veggen fra 08-04 står (FINN-hjelpesenteret bekrefter at lenker i annonsetekst ikke er klikkbare), men rangeringstabellen i 08-04 §3 manglet denne flaten. Video-lenken går til en **rå `.mov`-fil på eget domene** → vilkårlig URL, fri lenketekst, ingen host-whitelist. Og vertikalen er «Bolig til salgs» — **samme annonseskjema som hele bruktvolumet** (boligen er nybygg, men skjemaet diskriminerer ikke på boligens alder).
2. **Det reparerer Moat 2-hullet på FINN-flaten.** Prisen bokført 08-04 for karusell-veien var «et bilde gir null måling». En klikkbar lenke med `?src=finn` gir måling igjen — trykkbar i appen der boligkjøperen faktisk skroller, null dev hos noen. Self-serve-SharePanel har allerede `?src=finn`-lenken klar til å limes inn.
3. **Slotten står tom i praksis:** 6 av 6 vanlige bruktannonser sjekket (3 DNB, 2 EM1, 1 EIE) har bare standard-/kjedelenker → **ledig plass på hele bruktmarkedet.**
4. **Leverandør-spørsmålet (Andreas' oppfølging) er halvt besvart:** **Vitec Next bevist** — DNB-annonsen er Vitec (`bud.vitecnext.no/MSDNB/…`), lenkene er lagt inn per oppdrag med fri tekst; HEM/Nordvik/Aktiv kjører samme system. **Webtop/Visma (EM1, EIE) ubevist utenfra** — feeden deres pusher flere lenker til samme FINN-boks (Eiendommens hjemmeside / Bestill salgsoppgave / Visningspåmelding / Energiattest-PDF), så røret finnes, men alle observerte er kjedekonfigurerte standardtyper med identisk `cid=partner;;finn;;em1mn_…`-sporing — selv Frank Robert Baes eget Grilstad Marina-oppdrag har ingen fri lenke. Enten eksponerer ikke Webtop feltet for megler, eller EM1-konfigen gjør det ikke. **Avgjøres med ett skjermbilde fra Webtop** — god mikro-ask til Kristian/Frank Robert som samtidig tester viljen deres.
5. **Mulig gate hos FINN, uverifisert:** video i boligannonse krever «utvidet prospekt»-pakken (hjelpesenteret) → lenkefeltene kan være pakke-gatet snarere enn system-gatet.
6. **Risiko som skal med i enhver bruk:** FINNs annonseregler forbyr lenker til «FINNs konkurrenter» og markedsføring av andre tjenester — og FINN opererer selv Nabolagsprofil. En Placy-stedsside om boligens nabolag ligger i samme kategori som prosjektsiden DNB lenker til og bør stå seg, men flaten er leid: **kanalen er en oppgradering av FINN-flaten (bilde + klikkbar lenke), ikke fundamentet.** Visningsbekreftelse-, karusell- og QR-sporet består urørt.

**Begrunnelse:** 08-04-analysen kartla EM1s objektside og FINNs tekstfelt, men ikke annonsens strukturerte lenke-array — funnet myker opp «objektside-veggen»-konklusjonen for FINN-delen og gjenåpner målbar distribusjon på flaten med høyest trafikk. App/desktop-forskjellen Andreas så er rendering, ikke støtte: appen samler alle lenker i én boks, desktop sprer dem.

**Detaljer:** `2026-08-04-objektside-veggen-hem-og-nabolagsprofil-benchmark.md` §3b (verifikasjonsdata, leverandør-status, tabell-raden).

**Status:** Aktiv — Vitec-halvdelen validert, Webtop-halvdelen åpen (ett skjermbilde fra EM1-systemet avgjør). Åpent også: utvidet prospekt-gaten, og om FINN vil lese en nabolagslenke som konkurrent-lenke.

**Tillegg samme dag — 187-annonse-sveip (7 Sonnet-agenter) besvarte leverandør-spørsmålet så langt det kan besvares utenfra (§3b i 08-04-doccen har full tabell):**

1. **Webtop KAN fri lenke — men bare observert på prosjekt-annonser.** Beviset er en skrivefeil: EIE Sandvikas Villa Granheim-annonse har «Prosjektets hjemme**isde**» *(sic — menneske-tastet i fagsystemet)* + egendefinert **«Boligvelger»** → gjelstenbolig.no, i selve «Nyttige lenker». BOA (`boa.webtopsolutions.com`) → nebbaveien.no viser at det er plattform-funksjonalitet, ikke EIE-konfig. EM1 mn+nn har standardfeltet «Se prosjektets hjemmeside» → eksterne utbygger-domener på 4–6 prosjektannonser (bl.a. **Elvely/Bo Brøset → bobroset.no** — nok et Brøset-prosjekt i EM1s «Trondheim Øst»-geografi).
2. **På brukt er Webtop 0 av ~119** (EM1+EIE+PrivatMegleren samlet) — feltet ser prosjekt-gatet ut i Webtop-UI-et. Forvent «nei» på brukt i EM1-møtet; prosjektside-feltet er EM1-åpningen. Men faste felt kan peke eksternt også på brukt (én PM-annonse: «Se komplett salgsoppgave» → fredensborgbolig.no/vestbyen).
3. **Vitec-brukt-beviset står alene hos DNB.** Nordvik (MSNOP) og Krogsveen (MSKR) er bekreftet Vitec Next, men 28/30 hadde kun standardsettet; unntaket er Nordvik-brukt med «Social Video» → portal.diakrit.com (produktfelt, men ekstern URL på homes i praksis). Så: funksjonen finnes i Vitec, praksis/konfig varierer per kjede — HEM-svaret hentes i Vitec-UI-et (Einar).
4. **Fagsystem-biffangst:** Webtop er multi-tenant på subdomener (`em1.`, `em1nn.`, `eie.`, `pm.`, `boa.`, `partners.`) — **PrivatMegleren kjører Webtop, ikke Vitec** (antagelse falsifisert). EM1s 12 regionselskap deler ikke infrastruktur (mn/nn=Webtop, Sør-Rogaland=meglervisning, spor av Vitec-bud/Visma OneBroker) — «EM1 kjører Webtop» er en Midt-Norge-sannhet, ikke kjede-sannhet.
5. **Inkumbent-detalj:** EIE lister «Nabolagsprofil» → profil.nabolag.no som egen oppføring i Nyttige lenker — FINNs produkt sitter allerede i slotten. Placy-asken er «én oppføring til i samme liste».
6. **Konklusjon for distribusjonskartet:** prosjekt/nybygg-annonser = kanalen er åpen hos **begge** leverandører i dag (treffer utbygger-sporet og HEM-nybygg). Brukt = bevist kun DNB/Vitec; HEM sannsynlig men ubekreftet; EM1-brukt sannsynligvis stengt utenfra.

**Tillegg 2 samme dag — kort-kode-idé landet (Andreas): «Placy-kode» à la finnkoden.** 2–3 auto-genererte karusellbilder per eiendom (bildegeneratoren fra 08-04 §3, allerede byggekrav) får en kort, tastbar kode påtrykt: `placy.no/A424G` → riktig board.

1. **Koden løser bildets grunnproblem:** karusellbildet er trykk-dødt — en 4–5-tegns kode i URL-stien gjør det *tastbart* (ett steg; IKKE `find.placy.no` + søkefelt = to steg og forutsetter en vane ingen har; finnkoden virker fordi folk alt står i FINNs søkefelt). Kode-regler: case-insensitiv, dropp forvekslbare tegn (0/O, 1/l/I).
2. **Måling-ved-eksklusjon:** alle klikkbare flater har alt `?src=` — direkte-treff på `/KODE` uten src kan bare komme fra tastede flater (karusell, papir, salgsoppgave-tekst) → den umålbare flaten blir målbar uten egen mekanikk, og «et bilde gir null måling» (08-04 §3) er i praksis reparert. Gjør også Webtop-brukt-stengselet (pkt. 2 over) mindre kritisk: koden virker der lenkefeltet er stengt.
3. **Distribusjonen ER markedsføringen:** hvert bilde på hver annonse bærer `placy.no` — samme loop som bygde FINN-koden. Men i meglermøter selges *mekanismen* («kort adresse kjøperen kan taste fra bildet»), ikke navnet «Placy-kode».
4. **Byggekost nær null:** kode-tabell (kode → board, 301) i provisjonerings-flyten + koden inn i SharePanel og påtrykt bildegenerator-slides/caption. Tillegg til bildegeneratoren, ikke eget prosjekt. Designkravet fra §3 (stor type, quality(70), test på 390 px) gjelder koden mest av alt — det er den som skal tastes fra hukommelsen.

**Status tillegg 2:** Retning landet, ikke bygd — henger på bildegeneratoren (åpen beslutning 5 i 08-04-doccen).

**Tillegg 3 samme dag — Sundsøya (Inderøy): første konkrete full-pakke-case + det rurale asymmetri-argumentet (08-04-doccen §5f har full tekst):**

1. **Sundsøya (finnkode 468884345, planned-vertikal, salgsstart høst 2026) er HEM avd. Rosten** — Erik Ringseth Oxaal + Andreas Myran Steen; utbygger Sundsøya Eiendom AS, prosjektside `nyesundsoya.no` (WordPress). Annonsen bruker allerede «Nyttige lenker» → «Se prosjektets hjemmeside» → asken er «én lenke til». *Metodefunn:* FINN rendrer meglerkortet klientside — tekst-fetch av annonsen finner det ikke (Claude konkluderte først feil «ikke HEM»); meglerkort verifiseres i browser/skjermbilde.
2. **Full pakke definert:** embed på prosjektsiden + «Nyttige lenker»-lenke + karusellbilder + kort-kode = **én pris per prosjekt per salgsperiode**, fire flater, alle målt (`?src=embed`/`?src=finn`/kode-eksklusjon). Sveipen viste fri lenke på prosjekt-vertikalen hos begge leverandører → **full pakke er kjede-agnostisk på nybygg** — den ene flaten der hele pakken alltid er mulig.
3. **25–40k/salgsperiode for lite prosjekt er realistisk** — under sammenlignbare linjer i nybygg-markedspakken (film 6–12k, Marketer 50–200k) og 2–5 % av markedsbudsjettet. Friksjonen er behovsopplevelse («trenger vi dette for 30 boliger?» → svar: interessent-agn i forhåndssalg-fasen) og salgskanal (via megler, ikke kaldt mot utbygger) — ikke tallet. Internt: håndkuratert nivå 2 utenfor dekning = nær break-even; unntaksprising, betalingen er distribusjonsbevis + HEM-relasjon.
4. **Rural-asymmetri-argumentet (Aleksander har vært på det samme):** informasjonsasymmetrien er størst der dataene er tynnest; ukjent leses som risiko → ruralt prosjekt konkurrerer på pris uten nabolagsfortelling. FINNs eget «Nærområdet»-kart beviser det på skjerm: 2–3 pins på Sundsøya → området **ser tomt ut**; uniformt databaseprodukt skader rurale annonser, og Nabolagsprofil blir dårligere jo mer ruralt. **Ruralt er der inkumbenten er svakest og behovet størst** — posisjoneringsakse, ikke bare salgsargument. Sorterer inn i to-SKU-modellen (by = tetthet → nivå 1 → grunnpakke; ruralt = asymmetri → håndkuratert nivå 2 → prosjekt-SKU); Trondheim-only-beslutningen står (gjaldt automatisert pipeline). Motvekt: per-strøk-amortiseringen forsvinner ruralt — stykkgods, ikke skalering.
5. **Pitch-mekanikk:** side-ved-side-demo — FINNs Nærområdet-kart vs. Placy-board på samme adresse, skillet data/lokalkunnskap synlig på tre sekunder. Tilflytting-vinkelen gir megleren et argument *de* bruker mot utbygger. **Avd. Rosten = tredje dør inn i HEM** (i tillegg til Einar og Løbakk); §5c-advarslene gjelder konkret (to-SKU-skillet først, aldri per-enhet høyt).

**Status tillegg 3:** Case identifisert, ingen kontakt tatt. Full pakke henger på bildegeneratoren + kort-koden; embed på WordPress-utbyggerside er friksjonsfri (samme mønster som wesselslokka.no).
