...
> **✅ RESOLUSJON (audit-fiks 2026-07-05):** Fase-1-loggeren ble bygget (r13.1–r13.7) UTEN at konvolutt-beslutningen ble tatt — fanget av Fable-auditen som P0 sammen med to søsken-P0 (3 av 4 event-typer manglet `project_id`; `session_id` ble generert per kall → null økt-gruppering). Alle tre er nå fikset via `lib/instrumentation/engagement-scope.tsx`: ÉN emitter per board-mount bærer `project_id` + klient-generert økt-nøkkel (UUID v4, shape-validert server-side) + kontekst-konvolutt (`mode`, `has_3d_addon`, `categories_presented` i presentert rekkefølge, `locale`) i `payload.context` på ALLE fire emit-sites. `travel_mode`/`time_budget`/`map_center/zoom` er IKKE med i konvolutt-v1 — boardet har ingen global travel-mode-velger i dag; feltene er additive i jsonb når flatene finnes. Strøk bæres bevisst ikke i konvolutten: avledes stabilt ved aggregering via `project_id` → prosjektkoordinat → `find-area-for-point` (PRD 8). `route_requested`/`search_intent` står som eksplisitt deferred per rammen over (retning, ikke build-nå). Events logget FØR fiksen (19 rader, alle test-/verifikasjonsartefakter) mangler konvolutt+attribusjon og er ubrukelige som moat-data — wipe-beslutning ligger i DECISIONS-QUEUE.
