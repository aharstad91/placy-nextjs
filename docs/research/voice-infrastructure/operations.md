...
Shared-platform cutover completed on 2026-09-17: deployment `dpl_8RtoKwB491jQzDtwTPFCsRRwH6nZ`, application commit `11c78d1`. See [migration evidence](shared-platform-validation.md) for the bounded anonymous call and verified identities. The internal Vercel project name remains unchanged. The subsequent canonical-domain release is `dpl_3o3VM6GjRr2u2rxwCH67Eyvps8Ar`, from the application tree recorded in `c0a0b19`; see [project URL verification and rollback](project-url-validation.md).

## Deployment and access

- Project: `placy-nyhavna`, in the existing Vercel Pro team. Its internal name is retained, but the registry and `/[slug]` route make it one reusable runtime for multiple customer projects. The older `placy` main app remains a separate compatibility boundary.
- Canonical share link: https://placy.no/nyhavna — direct 200 on the shared platform, with the address retained. Every new project uses `https://placy.no/<slug>`. The apex domain belongs to the shared Vercel project; www remains on the unchanged older main app. No DNS change was needed. The old www `/nyhavna` redirect now targets the canonical apex (version `0022f206-e95b-4024-b653-10690fd999c9`).
- Shared route: `/<slug>`. Legacy `/p/<slug>` and `https://placy-nyhavna.vercel.app/demo/nyhavna-lokal` redirect relatively to the root slug and preserve query parameters. Neutral technical alias: https://placy-platform.vercel.app/nyhavna. Both aliases point to the same deployment; keep them for compatibility/rollback, but share the canonical placy.no URL. Only apex website paths redirect to www; other existing alias demos remain native.
