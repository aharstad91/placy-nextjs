...
import { loadDataset, assertReferences } from '@/lib/demo/nyhavna-lokal/dataset';
import { localPlacesSchema, type LocalDataset } from '@/lib/demo/nyhavna-lokal/schema';
...
describe('lokale kjøpesentre', () => {
  it('beholder servering når stemmen viser et senter fra en kafé', async () => {
    ...
    const b = buildLocalBoard(await fixture());
  ...
  it('viser senteret én gang og butikkene i eksisterende senterregister', async () => {
    const d = await fixture(); assertReferences(d);
    const b = buildLocalBoard(d);
  ...
  it('bevarer butikkens egne fakta, men sender kartet til senteret', async () => {
    const d = await fixture(), b = buildLocalBoard(d);
  ...
  it('henter senterets egne fakta selv når en butikk står først i datasettet', async () => {
    ...
    const knowledge = createNyhavnaKnowledge(buildLocalBoard(d), buildVoiceDeps(d).knowledge);
  ...
  it('peker manus og FAQ-lenker til den synlige sentermarkøren', async () => {
    ...
    const tour = createPresentation(createNyhavnaConversation(b, buildVoiceDeps(d)), d.board.presentation!, d.places, d.board.center);
  ...
  it('avviser ukjent forelder, selvreferanse og en forelder som ikke er senter', async () => {
    for (const parentPlaceId of ['ukjent','matbutikk','utenfor']) {
      ...
      expect(() => assertReferences(d)).toThrow(/parentPlaceId/);
    }
  });
  it('avviser kjedede sentre og udokumentert lite senterutvalg', async () => {
    ...
    expect(() => assertReferences(small)).toThrow(/minst fire/);
  });
});
