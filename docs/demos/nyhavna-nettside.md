...
2. **Tale på Vercel er ikke løst.** Chatflatens stemme finnes bare i den lokale Live-ruta (`/api/prototype/live` med serverens egen sidebandforbindelse i Node-prosessen og én samtidig sesjon per instans). Den er ikke kontrollert på Vercel, og `PLACY_NH_CHAT_VOICE=true` bør ikke settes der før den er det. Den delte Anja-tjenesten avviser `surface=chat` (hostet GET svarer 404) og kjenner verken chatflatens kartfrie verktøyliste/instruks (`lib/live/chat-surface.ts`), `session.input` fra et signert tekst-token eller opptaket for tale → tekst (`lib/demo/leangenbukta-chat/voice-handoff.ts`, `/api/prototype/live/handoff`). Ekte stemmeparitet på www krever derfor at Anja-tjenesten får: (a) chatflaten for `nyhavna-lokal` med samme verktøyfilter og instrukser, (b) verifisering av tekst-tokenet mot samme nøkkel og besøkende, (c) transkriptopptak og utstedelse av nytt token når talen slutter, (d) Nyhavna-målerens kvote i regnskapet, og (e) at www-widgetens bro snakker med tjenestens WebSocket-transport i stedet for den lokale ruta — eller at kopien flyttes til plattformen som `placy.no/<slug>` slik arkitekturreglene uansett krever for nye prosjekter.
