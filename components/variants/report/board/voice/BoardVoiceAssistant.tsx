...
type UiReference = {
  ...
  sources?: Array<{ id?: string; title?: string; label?: string; page?: string; url?: string; checkedAt?: string }>;
};
...
export function BoardVoiceAssistant({ compact = false }: { compact?: boolean }) {
  ...
  const realtime = useRealtime({
    ...
    snapshotId: (data as BoardData & { demoSnapshotId?: string }).demoSnapshotId,
  });
  ...
  const references = realtime.references as UiReference[];
  ...
  const showPlace = (reference: UiReference) => {
  ...
  return (
    <section aria-label="Placy-assistent" className="my-4 overflow-hidden rounded-[20px] border border-[#d8e5df] bg-[#f3f8f5] text-[#173b31]" data-testid="board-voice-assistant">
      <div className="flex items-center gap-3 px-3.5 py-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p role="status" aria-live="polite" className="mt-0.5 text-[11px] leading-4 text-[#4e6c60]">{statusText}</p>
        </div>
        {(running || messages.length > 0) && !error && <button type="button" onClick={startNewConversation} aria-label="Ny samtale" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white text-[#215d48] transition hover:bg-[#e3eee8]"><RotateCcw size={15} /></button>}
      </div>
      ...
      {expanded && <>
        ...
        {references.length > 0 && <div aria-label="Steder og kilder" className="mx-3.5 mb-2 space-y-2 rounded-xl border border-[#dce8e1] bg-white/75 px-3 py-2.5">
          <p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-[#698174]">Omtalt i svaret</p>
        ...
        {running && <div className="mx-3.5 mb-2 grid grid-cols-2 gap-1 rounded-xl bg-[#e2ece6] p-1" aria-label="Samtalemodus">
          <button type="button" onClick={() => chooseMode("text")} aria-label="Bytt til tekst" aria-pressed={realtime.mode === "text"} className={cn("flex min-h-8 items-center justify-center gap-1.5 rounded-lg px-2 text-[11px] font-semibold", realtime.mode === "text" ? "bg-white shadow-sm" : "text-[#557066]")}><Keyboard size={13} />Skriv</button>
          <button type="button" onClick={() => chooseMode("voice")} aria-label="Bytt til tale" aria-pressed={realtime.mode === "voice"} className={cn("flex min-h-8 items-center justify-center gap-1.5 rounded-lg px-2 text-[11px] font-semibold", realtime.mode === "voice" ? "bg-white shadow-sm" : "text-[#557066]")}><Mic size={13} />Snakk</button>
        </div>}
