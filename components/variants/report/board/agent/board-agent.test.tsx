...
vi.mock("@/components/variants/report/board/board-state", async (original) => ({
  ...
  useBoard: () => ({ data, state: boardState, dispatch, mapCamera: camera }),
}));
...
const voice = {
  ...
  hangUp: vi.fn(),
  sendText: vi.fn(),
  runTool: vi.fn(async () => ({ ok: true, shown: "x" })),
  linkAgent: vi.fn(),
};
...
describe("BoardAgentProvider", () => {
  ...
  it("med talen i gang går et stedsvalg til talen, ikke til tekstbanen", () => {
