...
**Produktvalget som rammer alt:** gå er default og modusen er noe leseren aktivt slår på. Andreas' formulering: «det er egentlig en utfordring, men jeg mener kanskje at det er en toggle/modus en må aktivere, for å vise 60min gange, når du kan kjøre i 15min». Bil-modus komprimerer stigen; gå er den ærligste rammen for et nabolag. Målt på boardet: Solbakken barnehage er 35 min å gå, 17 å sykle, 11 å kjøre — boardet fortalte bare det første tallet, og undersolgte dermed hver adresse som ikke er tett by. Det er Placys egen versjon av svakheten salgspitchen bruker mot FINNs Nærområdet.

**Leveranse:** 56 filer, +4 982/−546 linjer, 7 nye testfiler. **2 442 tester (fra 2 295), tsc rent, lint 0 errors, build passerer.** Reisetidene precomputes for alle tre profiler, så et modusbytte er et perspektiv-bytte uten nettverkskall; bare rutelinja hentes fra Directions. `travelMode` bor i board-reduceren og røres av INGEN navigasjons-action. `buildNeighbourhoodList` er modus-parametrisert med felt-rename (`walkMinutes`→`minutes`, `minWalk`/`maxWalk`→`minMinutes`/`maxMinutes`) — et gang-navngitt felt som bar biltid ville vært en felle for neste leser, og TypeScript fanget hele blast-radiusen mekanisk. Kategori-REKKEFØLGEN følger også modusen, som planen lot stå åpent.

**Backfill kjørt mot produksjon:** 1 514 av 1 514 POI-er på alle ni boards har nå gå, sykkel og bil. 0 hull. Idempotens verifisert mot prod (andre kjøring: 0 rader skrevet, alt uendret). ~4 500 Matrix-elementer, under 5 % av månedskvoten.

**Fem funn bare implementeringen kunne avdekke:**

1. **Moat 2 har vært dødt siden ~10. august.** `ReportReelsPage` sendte `project.id` som `projectId`, men det feltet er PRODUKT-UUID-en (`v2-queries.ts:330` mapper `product.id` inn i render-formens `id`), mens `logEventSchema` krever container-formen `{customer}_{slug}`. HVERT event på rapport-boardene ble avvist av valideringen — stille, fordi `logEvent` er fail-soft. Nyeste rad i `v2.events` var fra midtbyen, den eneste flaten som sendte riktig form. Pre-eksisterende og utenfor planen, derfor egen commit.

2. **Datatap jeg selv utløste under backfillen.** `update({ travel_times })` erstatter HELE jsonb-objektet. Da Mapbox rate-limitet sykkel mens bil lyktes, ble gangtiden slettet på 31 POI-er på Sundsøya. `mergeTravelTimes` slår nå sammen mot basen (friske verdier vinner, korrupte bæres ikke videre), og 429 retries med `Retry-After` framfor å hoppe over bolken. Alt gjenopprettet.

3. **422-bolke-bugen tatt på fersk gjerning.** Mapbox Matrix avviser en forespørsel med ett matrise-element. Martin Barstads veg har 97 POI-er og 97 ≡ 1 (mod 24) — nøyaktig ett punkt manglet gangtid, og fail-soft-kontrakten svelget det som en warning. `batchDestinations` garanterer nå 2–24 per bolk ved å låne fra forrige (24 + 1 → 23 + 2; sammenslåing er over koordinatgrensen).

4. **`contextEnvelope` i `event-schema.ts` er `.strict()` — tredje port planen ikke navnga.** Planen slo fast «ingen migrasjon, ingen `EVENT_TYPES`-bump» og hadde rett i begge, men et nytt konvolutt-felt som ikke også legges til i Zod-skjemaet stanser ALL event-logging stille. `travel_mode` er `.default("walk")` i skjemaet men obligatorisk i TS-typen: en klient på forrige bundle sender den gamle formen gjennom et deploy, og de eventene skal ikke droppes. Kommentar lagt i skjemaet til neste som utvider.

5. **Reduceren har TO steder som returnerer `initialBoardState`,** ikke ett: `RESET_TO_DEFAULT` og `BACK_TO_ACTIVE` uten aktiv kategori. Planen nevnte bare den første. Begge går nå via `resetNavigation`, og hver action er testet eksplisitt for at modusen overlever. I tillegg var `BoardMapControls` gatet på `has3dAddon && interactive`, så modusvelgeren ville vært usynlig på nøyaktig de boardsene funksjonen finnes for.

**Fire korreksjoner til planen, rettet i plandokumentet:** Grilstad Marina hadde INGEN hull (182/182) — 58 %-tallet kom fra et usidet PostgREST-oppslag, og `project_pois` har sammensatt PK så en siding sortert bare på `poi_id` dupliserer og hopper over rader. `MapPopupCard` er død kode, ikke live via `POIExploreModal` (`scripts/midtbyen/enrich-stores.ts` sier det rett ut) — behandlet som `ReportPOICard`. Det var SYV statiske «gangavstand»-påstander i fem filer, ikke tre; alle rettet, med en kilde-skannende vakt som fanger en åttende. Og `board-state.test.ts` fantes allerede — planen sa opprett `.tsx`, som ville gitt to testfiler for samme modul.

**Tre UI-bugs Andreas fant ved å bruke det, ikke testene — og hvorfor:** jeg verifiserte gjennom `element.click()`, som IKKE fyrer `pointerdown`/`mousedown`. Et ekte museklikk gjør det, og det er den bubblingen kart-laget reagerer på. Metodelærdommen er at kart-UI må verifiseres med full pointer-sekvens (eller ekte CDP-klikk), og at Mapbox-instansens `getSource`/`getPaintProperty` bør leses direkte — via React-fiberen — framfor å tolkes ut av skjermbilder. Da jeg først gjorde det, tok diagnosen minutter i stedet for tre runder med gjetting.

- **Kart-klikket leste chip-klikk som bakgrunnsklikk** → `BACK_TO_DEFAULT` → punktet lukket seg og chipen forsvant under fingeren. Chipen kan IKKE bruke markørenes vanlige `stopPropagation` (`BoardMarker:93`): den har interaktivt innhold, og et stoppet event når aldri Reacts delegerte handlere, så panelets modus-rader ville vært døde. Løst med `data-travel-chip` + `closest()`-filter i kart-klikket — samme mønster som `BoardMap3D:344` alt har for `gmp-click`.
- **POI-popupen dekket den kollapsede chipen.** `.mapboxgl-popup` ligger på z-20 i en SØSKEN-container av markørene, så chipens z-4 tapte. På korte ruter havner rutens midtpunkt under popup-kortet, og `elementFromPoint` i chipens sentrum returnerte `.mapboxgl-popup-content` — chipen var bokstavelig talt umulig å trykke på. Første forsøk fikset bare den ÅPNE tilstanden; nå ligger chipen over popupen i begge.
- **Chipen dekket ruta den beskriver.** `anchor="center"` la den rett på midtpunktet, og en 3-minutters biltur er ~110 px ved vanlig zoom mens chipen er 104 px bred. Ruta oppdaterte seg korrekt hele tiden — målt mot den live Mapbox-instansen byttet geometrien på 4 av 4 punkter og gjennom 5 bytter på rad (gå 43 → sykkel 58 → bil 26 koordinater, `line-opacity: 1`) — men den var usynlig under chipen, så modusbytte SÅ ut som ingenting. `anchor="bottom"` + offset −8.
- I tillegg: modusbytte fadet ut linja umiddelbart mens fade-inn bare lyttet på `routeData`. Kartet sto tomt gjennom hele hentingen. Modusbytte er ikke en fade-ut-grunn i det hele tatt — ruta går til samme punkt, bare en annen vei, så den gamle linja er sann til den nye ankommer.

**Konsolidering underveis:** `BoardRouteProvider` samler rutehentingen — rutelinja, chipen og 3D-ruten kalte `useRouteData` hver for seg (tre Directions-kall per POI-klikk, akseptert prototype-gjeld), og med modusbytte ville hvert bytte multiplisert dem. En kilde-vakt-test feiler hvis noen legger hooket tilbake i et kart-lag. `useTravelChip` deler innholdet mellom 2D- og 3D-chipen så motorene skiller seg i posisjonering, ikke i tall. Tidsmerket i 3D var en inline-SVG i et `Marker3DInteractiveElement` (templaten MÅ inneholde `<img>`/`<svg>`, så et utvidbart panel kunne ikke bo der) — nå et HTML-overlay etter `BoardPOI3DMiniPopup`-mønsteret, og «ÉN konsument av `projectLatLngToScreen`»-invarianten er oppdatert til to navngitte.
...
**Serie 1 — Hoxton Mini Press, «An Opinionated Guide to…»:** Uavhengig fotobok-forlag i East London, grunnlagt 2013, drevet av et ektepar (Martin Usborne + Ann Waldvogel). ~40+ London-titler + internasjonale (Paris, New York, Berlin, Lisboa, Edinburgh, Marseille, Brighton, Bristol, Margate). £10.95–12.95, 112 × 162 mm, ~176 sider. Nummererte oppføringer + falset kart med nummererte pins. **Tre overførbare funn:** (1) tematisk snitt slår geografisk fullstendighet — temaene er *intensjoner* («Free», «Calm», «Rainy Day», «Kids'», «Weird»), ikke kategorier; (2) **én navngitt forfatter per tittel** (Free London = Emmy Watts) — curator-modellen i print; (3) **ingen digital utgave eller app funnet** — boka *er* produktet (designobjekt, gavemarked, museumsbutikk), så de er en usannsynlig lisensgiver. Bifunn: tittel-listen er gratis, markedsvalidert etterspørselsdata, og hvilke titler som har fått nye utgaver (East London 4. utg., Vegan 3. utg.) viser hvilke temaer som har varig etterspørsel.

**Serie 2 — Pocket Mountains, «Weekend Walks»:** Skotsk turguide-forlag, helt annet DNA. *London: 30 Weekend Walks* av Patrick Kinsella (ISBN 9781907025891), *Edinburgh*/*Glasgow Weekend Walks* (25 turer hver). Format per tur: distanse · tid · start · finish med T-bane/tog-symboler, rutekart med prikkelinje, og turn-by-turn-prosa med lokalhistorie innvevd. **Dette er strukturelt nesten identisk med det `manus-curator` produserer — Pocket Mountains-formatet er Placy Guide + VO, på papir.** De to seriene kartlegger altså på to av våre tre produkter: Hoxton ≈ Report/Explorer (punkt-indeks), Pocket Mountains ≈ Guide (rute).

**Royal Parks-skiltet (The Regent's Park / Primrose Hill, © 2026, håndillustrert) — fem grensesnittmønstre:** (1) **Off-map-destinasjoner som kant-etiketter med gangtid og retningspil** («Paddington 30 minutes walk», «Baker Street 2 minutes», «Camden Lock Market 22 minutes») — motsatt av vår Time Budget, som *dimmer bort* det som ligger utenfor. Vurdert som skiltets sterkeste enkeltgrep. (2) **Skalalinje i minutter, ikke meter.** (3) **Begrensninger som ikoner:** no dogs / dogs on leads / steep gradient / no public access / cycling permitted — hva du får *gjøre*, ikke bare hva som er der. (4) **Tilgjengelighet som strukturert data:** fem distinkte toalett-typer (public/accessible/inclusive/children's only/Changing Places) + baby changing + AED + steep gradient. (5) **Sesong som attributt** — «(seasonal)» på kiosk, båtutleie, fluktstoler; i Norge en dramatisk viktigere akse (badeplass/akebakke, skøytebane, lysløype, brøytet/ubrøytet). Bifunn: tekstblokkene nederst er prosa **per tema, ikke per POI** — uavhengig validering av v3-manus-formatet, inkl. «Local landmarks» som beskriver ting *utenfor* parken (samme kontekst-bleed som bridgeText). Skiltet har QR + what3words-anker, dvs. det er selv en print→digital-bro, og «(seasonal)»-merkingen er dens egen innrømmelse av at trykk ikke kan si om kiosken er åpen i dag.

**Kodebase-funn (verifisert, ikke antatt):** `seasonal` og `accessibility` finnes alt i `lib/types.ts` (linje 642-644, 697-727), men **bare som redaksjonelle grounding-temaer** — altså temaer tekst genereres *om*, ikke strukturerte felt på POI-en. Eneste strukturerte temporale felt er `openingHoursJson` (Google-cachet). Vi har med andre ord alt bestemt at disse dimensjonene betyr noe, men kodet dem som tekst i stedet for data. Å løfte dem fra tema → attributt gir tre ting for én innsats: Moat-1-dybde FINN Nabolagsprofil ikke matcher, filterflate på boardet, og **Moat-2-instrument** (et klikk på «hundevennlig» eller «uten trapper» er et persona-signal prosa aldri kan gi — direkte i 06-28-linjen «UX er datainnsamlings-apparatet»).

**dyreparken.no/kart reverse-engineered (verifisert mot `main.js`, ikke gjettet):** Mapbox GL JS **v1.4.1**, tre lag. (1) Illustrasjonen er **ikke én JPG, men et raster-tilesett** — georeferert, kuttet i 256×256 PNG-fliser med alfa, servert fra eget domene: `addLayer({type:"raster", source:{type:"raster", tiles:["/tiles/{z}/{x}/{y}"], tileSize:256, minzoom:14, maxzoom:18}})`. De skarpe diagonale kantene er gjennomsiktighet rundt en tegning som selv er laget i skrå projeksjon, lagt på et nord-opp fotavtrykk. (2) POI-er som clustret GeoJSON oppå: `cluster:true, clusterMaxZoom:17`, hvite sirkler (`#ffffff`, r=14) + `point_count`-symbol. (3) **Kameraet er låst** — `dragRotate.disable()`, `touchZoomRotate.disableRotation()`, `NavigationControl({showCompass:false})`. Det er ikke stilvalg, det er *prisen*: tegningen ligger flatt på bakken med innbakt falskt perspektiv og avsløres i det kameraet vippes. Bifunn: `isCustomTileInBounds(z,x,y)` hardkoder eksakte flis-intervaller (z16: 5×4 fliser → z19: 40×32) for å unngå 404-storm utenfor parken, og `outsideofmap.png` er egen markør for POI-er utenfor det illustrerte området — dvs. de løste off-map-problemet med avvikende ikon, ikke kant-etikett med reisetid.

**Vurdering av illustrasjons-sporet (hvorfor det ikke er en åpenbar vei):** Teknikken er billig — Mapbox GL ligger alt i boardet, raster-overlay + clustering er en ettermiddag. **Kostnaden er tegningen, ikke koden.** Dyreparken amortiserer *én* illustrasjon på *én* lokasjon i evig tid; vi har N strøk, og håndillustrasjon per strøk overlever ikke grunnpakke-økonomien (300–800/listing). `placy-illustrations` (Wesselsløkka-akvarell via Gemini 3 Pro Image) kunne gitt sjarmen til nær null marginalkost, **men generert bildeflate er ikke geometrisk sann** — pins drifter av sine egne features, og for et board der «3 min til barnehagen» skal være troverdig er det en kredibilitetslekkasje mot vår egen posisjon vs. FINN. I tillegg kolliderer kamera-låsen med 3D-defaulten (ekte 3D + flythrough må ofres). Konklusjon: **alternativ estetikk for en annen modus, ikke en oppgraderingsvei for dagens board.** Der det kan bære: utbygger-uteområde med arkitektlevert tegning — én tomt, kontrollert geometri, 250k-segmentet, samme økonomi som Dyreparken.

**Norske korpora kartlagt (lisens-sjekket, som alternativ til å digitalisere andres bøker):** `lokalhistoriewiki.no` (Nasjonalbiblioteket — 88 012 artikler, 241 405 bilder, **CC-BY-SA**); Kulturminnesøk/Askeladden (Riksantikvaren — kulturminner med koordinater, ArcGIS REST, daglig oppdatert, **NLOD**); **Trondheim byleksikon** (Bratberg, Kunnskapsforlaget, 2. utg. 2008 — 5 000 oppslag over gater, plasser, broer, kirker, institusjoner; opphavsrettsbeskyttet men lisensierbart, og et strøk-indeksert korpus i presis Placy-form for den ene byen vi har committet til). Fakta er ikke opphavsrettsbeskyttet, så alt kan brukes som fact-feed inn i «Gemini henter, Fable skriver»-mønsteret uten lisens. Prior art: Atlas Obscura (31 000 steder) gikk **database-først, så bok**, og selger DMO-/merkevare-partnerskap — motsatt rekkefølge av Hoxton, som stoppet ved boka.

**Status: REN INSPIRASJON.** Ingen kode endret, ingen filer i `lib/` eller `app/` rørt, ingen DB-kall, ingen commits utover denne loggføringen. Ingen scope-beslutning tatt.

**Kandidat-ideer (ikke prioritert, ikke bestilt):** (1) **Kant-etiketter med reisetid for off-map-destinasjoner** — vurdert som størst effekt per innsats, treffer et reelt hull i Time Budget-UX-en. (2) **Betingelse/tilgjengelighet/sesong som strukturert POI-akse** — største moat-gevinsten, men datamodell-inngrep som fortjener brainstorm før plan. (3) Minutt-skalalinje på boardet (nesten gratis, reisetidene er precomputet). (4) Figur-grunn-styling: strøket i full metning, resten av byen avmettet men med gatenavn lesbart (map-styling, ikke illustrasjonsbudsjett — det ene grepet fra Royal Parks-estetikken som *er* gratis). (5) Trondheim byleksikon som Moat-1-akselerator, først som fact-feed. (6) Intensjons-tema som akse *over* kategoriene, oversatt til beboer-språk («med barn», «uten bil», «regnværsdag», «hund») — merk at London-guidenes turist-vinkel må vaskes bort, jf. beboer-perspektiv-regelen for Reels-manus.
...
- **KLP-flyttingen kjørt som to parallelle sonnet-agenter** (Fable orkestrerte — Andreas' bestilling): ferjemannsveien-10 (2D, nærings-temaer «Kontoret ligger mellom parker…», 284 POI-er i pool / 156 vist, reisetider 284/284, akseptanse nivå 2 grønn; Overpass-idrett ga HTTP 406 og ble fail-soft hoppet over) + teknostallen (3D, 76 steder vist) provisjonert + re-seedet + live-verifisert fra v2 med 0 console-feil. Screenshots: `parity-ferjemannsveien10-v2.jpeg` / `parity-teknostallen-v2.jpeg`.
...
**Samme dag, «ja, kjør videre» — KODE-TRIM-PR-EN LANDET (public-lesestien død i kode):**
- **Translations → v2 (migrasjon 072, re-kjørbar):** 404 rader kopiert med id-remap — 366 poi (place_id/nsr/osm-join), 30 rene tema-id-er, 8 produkt-komposit (`<v2-produkt-uuid>_<temaId>`). 0 report-rader var korrekt (alle 7 tilhører ikke-migrerte prosjekter, verifisert per prosjekt). Slug-æra-id-er (2 045 rader) dekkes av git-backupen. `translations.ts` → v2-skjema (any-casten død — v2.translations er typet). **MÅ re-kjøres per provisjonering av EN-boards, senest før drop.** Live-verifisert: «Rockheim is Norway's national museum of…» i stasjonskvartalet-payloaden.
- **Skjult bug rettet:** `inherit-area-editorial` (pipeline-steg 8) leste PUBLIC-boardet via gamle `getProductFromSupabase` — retargetet til `getProductFromSupabaseV2` (kommentaren i fila sa allerede at retargetingen ventet på read-path-arbeidet).
- **Sletting (dead-track + transitivt foreldreløst, import-graf-sweep til konvergens):** `app/for/**`, `app/trips/**`, `app/api/poi-trips`, variants/{explorer,portrait,trip}, gammel scroll-rapport (`rapport/`-ruten + ReportPage-treet inkl. grounding-RENDERERNE — grounding-datapipen består, board-rendering er fremtidig arbeid), `components/story/**`, `lib/store.ts` (gammel global Zustand — boardet har egne stores), `lib/data.ts`, trip-adapter/trip-schema/trip-errors, story-generator-scriptene + story-skrivefunksjonene i mutations.ts, MapAdapter-laget (mapbox-sentinelen i map-adapter.test.ts fyrte som designet), database.types.ts, supabase-barrelen, ~40 ubrukt UI/kart-komponenter. **Netto: 203 filer endret, +490/−31 363 linjer.**
- **Omskrevet:** eiendom-landing → v2 generation-pending-sjekk + redirect til `rapport-board`; event-landing → redirect til `board`; eiendom-layouten trimmet (hentet products/shortId uten å rendre dem). Proxy: /for-frysingen fjernet (alt 301-er), `/rapport` → `/rapport-board`, guide-suffiks → prosjektroten.
- **Typer:** Trip/Guide/scroll/gamification/branded/container-typene ut av lib/types.ts; `Story` uten sections/themeStories; døde Db-aliaser ut; TrailCollection + DiscoveryCircle + CameraConstraints BEHOLDT (levende). eslint: `no-img-element` = error overalt (warn-unntaket slettet med filene).
- **Bevisst beholdt:** `/api/travel-times` (PRD 11-klassifisert reference-only), `lib/instrumentation/engagement-stats.ts` (Moat-2-leser, venter på dashboard), `lib/supabase/collections.ts` («Min samling» på event-boardet — lever til drop, se §4b).
